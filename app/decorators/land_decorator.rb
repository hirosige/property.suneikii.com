class LandDecorator < Draper::Decorator
  ...
  def decollated_status
    ...
    if self.status == 'secret'
      "<div class='label label-warning'>#{h.t("admin.lands.items.status.#{self.status}")}</div>".html_safe
    else
      "<div class='label label-primary'>#{h.t("admin.lands.items.status.#{self.status}")}</div>".html_safe
    end
