class CondoDecorator < Draper::Decorator
  delegate_all
  ...
  def decollated_status
    ...
    if self.status == 'secret'
      "<div class='label label-warning'>#{h.t("admin.condos.items.status.#{self.status}")}</div>".html_safe
    else
      "<div class='label label-primary'>#{h.t("admin.condos.items.status.#{self.status}")}</div>".html_safe
    end
